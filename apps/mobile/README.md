...
## Что дальше (по плану, этапы 3–6)
- Нативный чекаут (адрес → доставка → оплата ЮKassa через WebBrowser).
- Личный кабинет (вход/регистрация, заказы, избранное).
- Push-уведомления, иконка/сплэш из бренда, сборка и публикация (EAS).

Пока чекаут открывает сайт (если задан `EXPO_PUBLIC_STOREFRONT_URL`).
